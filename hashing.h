...
public:
	...
	int getMemoriaGasta(); // retorna a memoria utilizada
private:
	int tam; // tamanho da tabela
	Lista *tabela; // tabela de dados
	long int colisoes; // quantidade de colisoes
	int funcaoHashing(DuplaMovies valor); // calcular posicao
	int memoriaGasta; // memoria utilizada pelo hash
};
...
public:
	...
	~SLinear(); // destrutor
	long int qtdColisoes(); // retorna qtd de colisoes
	void inserir(DuplaMovies valor); // insercao de valores
	int getPosOcupadas(); // retorna qtd de posicoes ocupadas
private:
	int tam; // tamanho da tabela
...
private:
    ...
    DuplaMovies *vetDupla; //vetor de movies
	int* vetEstado; // parte da tabela com TAG estado da posicao(-1:vazio)
    long int colisoes; // numero de comparacoes realizadas

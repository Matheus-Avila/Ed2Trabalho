...
void pagina::inserirNaFolha(chave* chaveNova, int* comparacoes, int* trocas){
    //Percorre a partir do final, buscando a posicao da chaveNova
    *comparacoes = *comparacoes+1;
    for(int i = posOcupadas-1; i >= -1; i--){*comparacoes = *comparacoes + 1;
        //i = -1 indica que i percorreu a pagina e nao achou uma chave menor que chaveNova,
        //logo, a chave nova deve ser inserida na posicao 0
        *comparacoes = *comparacoes + 1;
...
void pagina::splitPagina(int* comparacoes, int* trocas){
    *comparacoes = *comparacoes + 1;
    if(eRaiz()){*trocas = *trocas + 1;
        ...
        pagina *filhaNova = new pagina(getOrdem(), this->getPai());*trocas = *trocas + 1;
        int i;*comparacoes = *comparacoes + 1;
        for(i = getOrdem()+1; i < getPosOcupadas(); i++){*comparacoes = *comparacoes + 1;
            filhaNova->chaves[i-getOrdem()-1] = this->chaves[i];*trocas+=2;
        ...
        *comparacoes = *comparacoes + 1;
        if(!this->NoIntermediario){
            *comparacoes = *comparacoes + 1;
            for(int i = getOrdem()+1; i < getPosOcupadas(); i++){*comparacoes = *comparacoes + 1;
                filhaNova->chaves[i-getOrdem()-1] = this->chaves[i];*trocas = *trocas + 1;
                this->posOcupadas--;
                filhaNova->posOcupadas++;
        ...
        //Caso o split seja em uma pagina intermediaria
            ...
            //passar ultimo ponteiro
            filhaNova->paginasFilhas[i-getOrdem()-1] = this->paginasFilhas[i];*trocas = *trocas + 1;

...
ESeparado::ESeparado(int tamanho)
{
    ...
    this->colisoes = 0;
	this->memoriaGasta = 2*tam;//vetor de moviesID(int) e userID(int) sao gastos fixos 
}
...
int ESeparado::getMemoriaGasta(){
	return this->memoriaGasta;
}
...
void ESeparado::inserir(DuplaMovies valor){
	...
	if(!this->tabela[posicao].listaVazia()){
		...
		if(testaIguais == 0){
        	colisoes++;
			this->tabela[posicao].inserir(valor); // insere o valor no final da lista que esta relacionada na tabela
			memoriaGasta= memoriaGasta+3; // Uma duplaMovies é inserida na memoria e o no proximo
		}
    }else{
		this->tabela[posicao].inserir(valor); // insere o valor no inicio da lista que esta relacionada na tabela
		
	}
	//if(testaIguais== 0){ return true} else{ return false}
}
...
void ECoalescido::inserir(DuplaMovies valor){
	...
	int auxColisao = colisoes; //Se tiver DuplaMovies iguais "anulamos" a contagem de colisoes
	...
	if(this->vetEstado[posicao] == -2){
		...
		if(this->vetEstado[posicao] == -1){
			aux = tam-1;
			colisoes++;
			if(this->vetDupla[posicao].getMovies() == valor.getMovies() &&
			this->vetDupla[posicao].getUser()== valor.getUser()){
				
				colisoes--; // A colisao que foi adicionada na linha 245 precisa ser cancelada
				//return false;
			}
...
long int SLinear::qtdColisoes(){
    return this->colisoes;
}

//retorna qtd de posicoes preenchidas na hash
int SLinear::getPosOcupadas(){
	return 3*this->posOcupadas;
}
...
void SLinear::inserir(DuplaMovies valor){
    ...
    if(posOcupadas >= this->tam){
        ...
        this->vetEstado[posicao] = 1; // define que existe um elemento inserido nessa posicao
		posOcupadas++;  // adiciona uma dupla na memoria e altera vetEstado
    }
}
...
// Retorna a quantidade de compara��es.
long int SQuadratica::qtdColisoes(){
    return this->colisoes;
}
...
// retorna qtd de posicoes ocupadas	
int SQuadratica::getPosOcupadas(){
	return 3*this->posOcupadas;
}
...
DHashing::DHashing(int tamanho)
{
    ...
    this->posOcupadas = 0; 
    this->colisoes = 0;
	for(int i=0; i < this->tam; i++){//Inicializacao da tabela
		this->vetEstado[i] = -1;
	}
}
...
// retorna qtd de posicoes ocupadas	
int DHashing::getPosOcupadas(){
	return 3*this->posOcupadas;
}

//Calcula a primeira posição de cada valor.
